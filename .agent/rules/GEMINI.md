...
1. Identify domain & agent → Read agent `.md` file in `.agent/agents/`
2. Announce `🤖 Applying knowledge of @[agent]...`
3. Load required skills from agent frontmatter
4. Apply domain rules (e.g. Purple Ban, Clean Code)
...
> 🔴 **MANDATORY:** Khi tạo module mới, thêm Collection DB, hoặc sửa đổi public API / business logic:
>
> 1. **Cập nhật `a-agentic/features/<feature>/`**: Đảm bảo đủ bộ 4 file (`dependencies.md`, `rules-and-flows.md`, `tech-spec.md`, `dev-history.md`).
> 2. **Cập nhật Dev History**: Sau mỗi lần hoàn thành tính năng hoặc sửa lỗi, bắt buộc ghi lại root cause, giải pháp và gotchas vào `dev-history.md`.

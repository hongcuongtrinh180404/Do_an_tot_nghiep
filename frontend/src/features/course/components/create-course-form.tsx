...
export function CreateCourseForm(): React.JSX.Element {
  ...
  const onSubmit = (data: CreateCourseFormData) => {
    // Development preview mode (API will be connected in next milestone)
    console.log('[CreateCourseForm:Submit]', data);

    toast.success('Thông tin khóa học hợp lệ! (Chế độ xem trước UI)', {
      description: `Khóa học "${data.title}" đã sẵn sàng kết nối API ở Milestone sau.`,
    });
  };
